...
const SearchForm = (props) => {
  ...
  const submitHandler = (event) => {
    ...
    const userSearch = props.data.filter((user) => {
        return (user.firstName + "" ).toLowerCase().indexOf(results.toLowerCase()) !== -1
    ...
    //   return
    //   user.firstName.indexOf(results) !== -1;

    // //   if (user.firstName.toLowerCase === results.firstName.toLowerCase) {
    // //     return results;
    //     // user.firstName.toLowerCase().indexOf(results.firstName().toLowerCase()) !== -1
